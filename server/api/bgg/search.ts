...
async function getGameData(searchTexts: String[]) {
  ...
  for (let i = 0; i < searchTexts.length && !id; i++) {
    ...
    if (ret.items.item) {
      id = ret.items.item['@_id'] || ret.items.item[0]['@_id']
    }
  }
...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (!searchTexts && !gameId) {
      return { result: { query: 'title or gameId must be a string' } };
    }

    if (!gameId) {
      gameId = await getGameData(searchTexts);
    }
    ...
    const res = await $fetch(`${apiURL}/thing`, { query: { id: gameId, stats: 1, } });
